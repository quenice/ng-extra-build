...
function buildi18n({env, buildInfo}) {
    printSubTitleLog('Build i18n start...');
    ...
    for (let extraFile of extraFiles) {
        ...
        const targetFilePath = `${target}/${extraFile}`;
        const sourceFileContent = require(sourceFilePath);
        Object.assign(sourceFileContent, extraFileContent);
        fs.writeFileSync(`${targetFilePath}`, JSON.stringify(sourceFileContent));
        console.log(`Merge [${sourceFilePath}] and [${extraFilePath}] to [${targetFilePath}]`);
    }
    printSubTitleLog('Build i18n finish!');
}
